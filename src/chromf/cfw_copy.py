...
# ------------------------------------------------------------------------
try:
    from chromf.spectrum_loader import (
        channel_products as _channel_products,
        _load_tca as _load_tca,
    )
except ModuleNotFoundError:   # tests that prepend src/
    from spectrum_loader import channel_products as _channel_products  # type: ignore
    from spectrum_loader import _load_tca as _load_tca                 # type: ignore

# … after SENSOR_RESPONSE …
_TCA = _load_tca()[:, 1]                 # Δx base curve (µm), same λ grid
TCA_SHIFT = _TCA.astype(np.float64)      # ensure float64 for numba
...
def _edge_response_jit(
    ...
    delta_shift: np.ndarray,          # NEW
    f_number: float,
    psf_kind: Literal["geom", "gauss"],
) -> float:
    """Monochrome edge value at (*x*, *z*) with LCA + TCA."""
    denom = _sqrt(4.0 * f_number ** 2.0 - 1.0)
    acc = 0.0

    for n in range(chl_curve.size):
        x_eff = x - delta_shift[n]                  # ← TCA shift
        rho   = _fabs((z - chl_curve[n]) / denom)   # LCA blur radius
        weight = _geom_esf(x_eff, rho) if psf_kind == "geom" else _gauss_esf(x_eff, rho)
        acc += sensor[n] * weight
    ...
    return _exposure_curve(acc / norm, slope) ** gamma
...
def edge_response(
    ...
    *,
    ...
    field_scale: float = 0.0,                 # NEW  (0 = no TCA)
    ...
    * ``psf_mode`` is either ``"geom"`` (pillbox) or ``"gauss"`` (Gaussian).
    ...
    delta_shift = field_scale * TCA_SHIFT     # scale curve to current field height

    return _edge_response_jit(
        float(x_px), float(z_um),
        slope, gamma_val,
        sensor, chl_curve_um.astype(np.float64),
        delta_shift,                            # NEW
        float(f_number), psf_mode,
    )
...
def detect_fringe_binary(
    ...
    *,
    ...
    gamma: float | None = None,
    chl_curve_um: np.ndarray,
    field_scale: float = 0.0,          # ← NEW
    f_number: float = DEFAULT_FNUMBER,
    psf_mode: Literal["geom", "gauss"] = DEFAULT_PSF_MODE,
) -> int:
    r = edge_response("R", x_px, z_um, exposure_slope=exposure_slope, gamma=gamma,
                      chl_curve_um=chl_curve_um, field_scale=field_scale,
    ...
    return int(
        sum(
            detect_fringe_binary(
                ...
                chl_curve_um=chl_curve_um, field_scale=field_scale,  # ← pass through
                f_number=f_number, psf_mode=psf_mode,
            )
            for x in xs
        )
